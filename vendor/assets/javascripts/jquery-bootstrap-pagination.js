		...
		*/
		...
		return PaginationView = (function() {
			...
			PaginationView.prototype.pages = function() {
				var buf, current_page, max, page, pages, total_pages, _i, _j, _k, _l,
					_m, _ref, _ref1, _ref2, _ref3;
				...
				if (total_pages > 10) {
					...
					if (current_page === total_pages) {
						for (page = _i = _ref = total_pages - max; _ref <= total_pages ? _i <=
							total_pages : _i >= total_pages; page = _ref <= total_pages ? ++_i :
							--_i) {
							pages.push(page);
						}
					}
					if (total_pages - current_page < max - 1) {
						for (page = _j = _ref1 = total_pages - max; _ref1 <= total_pages ?
							_j <= total_pages : _j >= total_pages; page = _ref1 <= total_pages ?
							++_j : --_j) {
							pages.push(page);
						}
					} else if (current_page > max - 1) {
						buf = max / 2;
						for (page = _k = _ref2 = current_page - buf, _ref3 = current_page +
							buf; _ref2 <= _ref3 ? _k <= _ref3 : _k >= _ref3; page = _ref2 <=
							_ref3 ? ++_k : --_k) {
							pages.push(page);
						}
					} else if (current_page <= max - 1) {
						for (page = _l = 2; 2 <= max ? _l <= max : _l >= max; page = 2 <=
							max ? ++_l : --_l) {
							pages.push(page);
						}
					}
					...
					for (page = _m = 1; 1 <= total_pages ? _m <= total_pages : _m >=
						total_pages; page = 1 <= total_pages ? ++_m : --_m) {
						pages.push(page);
					}
				}
				return pages;
			...
			PaginationView.prototype.render = function() {
				...
				$("[data-page=" + this.settings.current_page + "]", this.el).closest(
					"li").addClass("active");
			...
			PaginationView.prototype.isValidPage = function(page) {
				return page > 0 && page !== this.settings.current_page && page <= this
